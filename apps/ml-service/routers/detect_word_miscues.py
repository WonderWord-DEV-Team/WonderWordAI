...
from services.wav2vec_service import Wav2VecModelLoadError, detect_word_miscues

router = APIRouter()


@router.post("/detect-word-miscues")
async def detect_passage_word_miscues(
    ...
    #
    # Sibling of /detect-miscue: that one scores a whole short clip against
    # one target word (the practice-tab check). This one reports WHICH words
    # in a full passage were misread, for the reading-session flow.
    _ = x_internal_key
