  ...
  return (
    <header className="flex items-center justify-between px-6 sm:px-10 py-5 bg-[#FDF8F2]">
      ...
      <nav className="hidden sm:flex items-center gap-8 text-sm font-bold text-gray-700">
        {navItems.map((item) => (
          <a
            ...
            className={
              activeTab === item.key
                ? "text-[#E8604F] border-b-2 border-[#E8604F] pb-1"
                : "hover:text-[#E8604F]"
            }
          >
            {item.label}
      ...
      <div className="flex items-center gap-3">
        <span className="flex items-center gap-1 bg-white rounded-full px-3 py-1.5 text-sm font-bold text-gray-700 shadow-sm">
          ⭐ {stars.toLocaleString()}
        </span>
        <span className="flex items-center gap-2 bg-white rounded-full pl-1 pr-3 py-1 shadow-sm">
