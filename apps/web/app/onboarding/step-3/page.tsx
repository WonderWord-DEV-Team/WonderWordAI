...
export default function OnboardingStepThree() {
  ...
  return (
    <OnboardingShell step={3} narrow={false}>
      <h1 className="ffont-serif text-4xl font-bold text-[#a3352b] md:text-3xl">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        {GUARDRAILS.map(({ icon: Icon, title, body }) => (
          <div
            key={title}
            className="rounded-2xl border border-[#e8c84a] bg-[#fdf3d8] p-5"
          >
            ...
            <p className="mt-1.5 text-sm leading-relaxed text-[#6b5d33]">
              {body}
            </p>
          </div>
        ))}
      ...
      <div className="mt-8 flex items-center justify-end gap-3">
        ...
        <button
          ...
          className="rounded-full bg-[#a3352b] px-6 py-3 text-sm font-semibold text-white shadow-sm transition hover:bg-[#8c2c23]"
        >
          Continue →
        </button>
      </div>
