...
Creates or returns cached generated narration for a stored story. This route is separate from child
reading audio: the browser sends only the story ID, the route loads trusted `generated_stories.story_text`
from Supabase, verifies story visibility through the existing child/linked-parent RLS model, and
calls the internal ML `/narrate` endpoint only on a cache miss.
...
- `ML_SERVICE_URL`
- `ML_SERVICE_KEY`
- `STORY_NARRATION_BUCKET` (default `story-narrations`)
- `STORY_NARRATION_SIGNED_URL_TTL_SECONDS` (default `3600`)
- `TTS_PROVIDER`, `TTS_VOICE`, `TTS_MODEL`, `TTS_SPEAKING_RATE` on both web and ML services for cache versioning
...
{
  "data": {
    "storyId": "40000000-0000-4000-8000-000000000010",
...
Narration audio is synthetic and is stored in the private Supabase Storage bucket
`story-narrations` under `{childId}/{storyId}/{narrationVersion}.{extension}`. The browser never
receives `audio_path` and never uploads to this bucket; it receives a signed URL generated by the
server. Narration metadata is stored in `public.story_narrations`, which has RLS enabled and no
direct browser grants.

The cache key is a SHA-256 narration version derived from the story text hash, TTS provider, voice,
model, speaking rate, and alignment version. If a matching row and storage object can be signed, the
route returns immediately without rerunning TTS or WhisperX. When the story text or narration config
changes, the route stores the new version and removes obsolete metadata/audio after the new response
is available.

The internal ML endpoint is:

- Endpoint: `POST ${ML_SERVICE_URL}/narrate`
- Header: `X-Internal-Key: ${ML_SERVICE_KEY}`
- JSON body: `{ "story_id": "...", "text": "..." }`
- Response fields: `audio_base64`, `mime_type`, `duration`, `words`

No production TTS provider has been selected in this repository. The pipeline, storage, cache,
validation, and WhisperX alignment path are implemented, but production `/narrate` returns a
configuration error until a provider-backed `NarrationProvider` is added. The smallest next change is
to choose the provider, add its SDK/credential, and implement `get_narration_provider()` in
`apps/ml-service/services/narration_service.py`.
...
cd ../..
PYTHONDONTWRITEBYTECODE=1 pytest -q apps/ml-service/tests/test_narrate.py
